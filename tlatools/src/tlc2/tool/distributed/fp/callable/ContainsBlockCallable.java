...
public class ContainsBlockCallable extends FPSetManagerCallable {
	...
	public ContainsBlockCallable(FPSetManager fpSetManager, List<FPSets> fpSets, List<List<Fingerprint>> fps, int index) {
	...
	public BitVectorWrapper call() throws Exception {
		try {
			BitVector bv = fpset.get(index).containsBlock(fps.get(index));
