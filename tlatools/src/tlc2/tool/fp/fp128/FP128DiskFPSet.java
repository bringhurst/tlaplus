...
public abstract class FP128DiskFPSet extends NoBackupFP128FPSet implements FPSetStatistic {
	 ...
	 */
	protected static final int LogLockCnt = Integer.getInteger(FP128DiskFPSet.class.getName() + ".logLockCnt", 10);
	/**
	...
	protected AtomicLong diskWriteCnt = new AtomicLong(0);
	protected AtomicLong diskSeekCnt = new AtomicLong(0);
	protected AtomicLong diskSeekCache = new AtomicLong(0);
	
	// indicate how many cp or disk grow in put(long) has occurred
	protected int checkPointMark;
	 ...
	 */
	protected FP128DiskFPSet(final FPSetConfiguration fpSetConfig) throws RemoteException {
		super(fpSetConfig);
	...
	public final void init(int numThreads, String aMetadir, String filename)
		...
		final String propMetaDirPrefix = System.getProperty(FP128DiskFPSet.class.getName() + ".metadirPrefix");
	...
	public final boolean put(Fingerprint fp) throws IOException {
		...
		// next, look on disk
		boolean diskHit = this.diskLookup(fp128);
		...
		final Lock w = rwLock.getAt(getLockIndex(fp128)).writeLock();
		w.lock();
		
		// if disk lookup failed, add to memory buffer
		if (this.memInsert(fp128)) {
			w.unlock();
			this.memHitCnt.getAndIncrement();
			return true;
		}
	...
	protected abstract int getLockIndex(FP128 fp);
	...
	protected final void writeFP(RandomAccessFile outRAF, FP128 fp)
			throws IOException {
		fp.write(outRAF);
	...
	public final void recover(String fname) throws IOException {
		...
		FP128 fp = null;
		try {
			FP128 predecessor = null;
			while (true) {
				fp = (FP128) FP128.Factory.getInstance().newFingerprint(chkptRAF);
				this.writeFP(currRAF, fp);
				// check invariant
				Assert.check(predecessor == null || predecessor.compareTo(fp) == -1, EC.SYSTEM_INDEX_ERROR);
				predecessor = fp;
			}
